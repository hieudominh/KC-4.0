...
class TokenAcquirer(object):
    ...
    translate.google.com uses a token to authorize the requests. If you are not Google, you don't have this token
    and will have to pay for use.
    This class is the result of reverse engineering on the obfuscated and minified code used by Google to generate
    such token.

    This module re-used from py-googletrans library
    (https://github.com/ssut/py-googletrans)

    The token is based on a seed which is updated once per hour and on the text that will be translated.
    Both are combined - by some strange math - in order to generate a final token (e.g. 744915.856682)
    which is used by the API to validate the request.
    ...
    def _lazy(self, value):
        """
        Like lazy evaluation, this method returns a lambda function that returns value given.
        We won't be needing this because this seems to have been built for code obfuscation.
